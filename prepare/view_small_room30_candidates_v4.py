#!/usr/bin/env python3
"""Visually compare saved v4 attempts, including rejected ones. No Torch/model load."""
import argparse
import hashlib
import io
import json
...
class Evidence:
    def __init__(self,run,base):
        ...
        if report.get('status') not in ('TARGETED_V4_STOPPED_REJECTED','TARGETED_V4_FINISHED_REVIEW_REQUIRED'):
            raise ValueError('Need completed/stopped v4 evidence, not a running experiment')
        if bound['policy']['version']!='small_room30_targeted_v4':raise ValueError('Not v4 evidence')
        ...
        if (digest(base.read('manifest.json'))!=bound['evaluation_manifest_sha256']
                or manifest['checkpoint_sha256']!=bound['source_checkpoint']['sha256']):
        ...
        self.stages['Original LoRA 1560']=dict(folder=None,receipt=None,rows=None)
        self.default_stage='Original LoRA 1560'
    ...
    def arrays(self,stage,case,reference='Original LoRA 1560'):
        ...
        if selected['folder'] is None:raw=original['adapted_raw']
        else:
            if key not in self.cache:
                ...
                if not np.array_equal(a['source_raw'],original['adapted_raw']):raise ValueError('Candidate/source pairing mismatch')
                self.cache[key]=a['adapted_raw']
            raw=self.cache[key]
        return dict(points=original['points'],gt=original['gt'],adapted_raw=raw,
            base_raw=original['adapted_raw'] if reference=='Original LoRA 1560' else original['base_raw'])
...
def main():
    ...
    p.add_argument('--baseline-zip',type=Path,help='Optional full_review01 ZIP instead of the original1560 folder')
    p.add_argument('--verify-only',action='store_true')
    p.add_argument('--host',default='127.0.0.1');p.add_argument('--port',type=int,default=8092)
    a=p.parse_args()
    run=Store(a.run_summary,a.review_zip,'outputs/small_room30_targeted_v4_run01/')
    ...
    try:
        ...
        evidence.verify_all()
        print('[VERIFIED] %d saved paths; %d selections including original1560.'%(len(evidence.cases),len(evidence.stages)),flush=True)
        ...
        server.gui.add_markdown('## Saved v4 candidates — visual review\n\n'
            'Repair scope: Sit in0205,0403,0603. You judge target visibility; other paths are preservation checks.\n\n'
            '**16 saved paths only:4 target paths +12 preservation paths. Not a full30-room v4 evaluation.**\n\n'
        ...
        reference=server.gui.add_dropdown('Reference',options=('Original LoRA 1560','Frozen Base'),initial_value='Original LoRA 1560')
