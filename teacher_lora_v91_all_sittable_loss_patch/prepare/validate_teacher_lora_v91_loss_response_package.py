#!/usr/bin/env python3
"""Verify the Teacher-v9.1 active-support loss-response delivery."""
...
def main() -> None:
    ...
    if (
        value.get("schema")
        != "teacher_lora_v91_all_sittable_loss_response_package_v1"
        or value.get("status") != "PACKAGE_PASS"
        or not isinstance(authorization, dict)
        or authorization.get("loss_response_grid") is not True
        or authorization.get("candidates") != 3
        or authorization.get("updates_per_candidate") != 3
        or authorization.get("checkpoint_export") is not False
        or authorization.get("corrected_overfit_after_validated_pass") is not True
        ...
        != ["active_support_equal_macro", "within_object_hotspot_ranking"]
    ):
        raise ValueError("Teacher-v9.1 response package policy changed")
    expected = value.get("files_sha256")
    if not isinstance(expected, dict) or not expected:
        raise ValueError("Teacher-v9.1 package inventory is absent")
    actual = {
    ...
    if actual != set(expected):
        raise ValueError(
            "Teacher-v9.1 package inventory changed: "
    ...
    for relative, expected_hash in sorted(expected.items()):
        ...
        if actual_hash != expected_hash:
            raise ValueError("Teacher-v9.1 package hash mismatch: " + relative)
    print("[PACKAGE_PASS] Teacher-v9.1 active-support loss-response delivery")
