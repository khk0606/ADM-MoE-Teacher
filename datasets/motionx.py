...
class MotionXDataset(Dataset):
    ...
    def __getitem__(self, idx): # scene, motion, contact map, text, etc.를 가져오는 함수
...
class ContactMapCustomDataset(ContactMapDataset):
    ...
    def __getitem__(self, idx):
        ...
        s, i, scene_id, scene_trans, desc = self.all_data[index]
        scene_trans = -scene_trans
        scene = np.load(os.path.join(self.data_dir, f'{s}/points/{i:0>4}.npz'))#scene 정보 불러오기
        points = scene['points'].astype(np.float32)
        ...
        data = {
            'x': contact,
            ## conditions #추후 나중에 여기 부분에 input condition 추가
            'c_pc_xyz': xyz,
            'c_pc_feat': feat,
            'c_text': text,
            ## for visualization or saving results
