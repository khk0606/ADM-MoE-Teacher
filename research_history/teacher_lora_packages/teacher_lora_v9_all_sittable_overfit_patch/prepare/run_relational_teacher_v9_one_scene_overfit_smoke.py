#!/usr/bin/env python3
"""Fresh one-scene learnability smoke for Teacher-v9 all-sittable GT.
...
from relational_teacher_v9_overfit_smoke_contract import (  # noqa: E402
    DEVELOPMENT_SCENE,
    GRAD_CLIP,
    HELDOUT_TRAIN_SCENE,
    LEARNING_RATE,
...
def main() -> None:
    ...
    if not args.device.startswith("cuda:") or not torch.cuda.is_available():
        raise RuntimeError("Teacher-v9 overfit smoke requires CUDA")
    if (
        ...
        raise ValueError("Teacher-v9 overfit smoke hyperparameters are sealed")
    configure_reproducibility(args.seed)

    output_dir = args.output_dir.expanduser().resolve()
    if output_dir.exists():
        raise FileExistsError("refusing to overwrite Teacher-v9 overfit smoke")
    ...
    for step in range(1, args.steps + 1):
        ...
        total = (
            ...
            * objective["paired_prompt_invariance"]
            + OBJECTIVE_WEIGHTS["frozen_v5_replay_preservation"] * replay
        ...
        if step == 1 or step % 10 == 0:
            per_instance = objective["per_instance_primary"].detach().mean(dim=0).cpu()
            print(
                f"[OVERFIT] step={step:03d}/{args.steps} "
                f"total={float(total.detach().item()):.6f} "
                f"bed={float(per_instance[0]):.6f} "
                f"chair={float(per_instance[1]):.6f} "
                f"high={float(per_instance[2]):.6f} grad={grad_norm:.6f}",
                flush=True,
            )
    ...
    # Prefer the earliest train-only state proving all three objects.  This
    # state lives in RAM only and is never exported as a checkpoint.
    selected = eligible_states[0] if eligible_states else {
        "step": args.steps,
        "state": _lora_cpu_state(named_lora),
    ...
    initial_seed, reverse_seed = _stable_seeds(
        args.seed, "one_scene_overfit_rollout", TRAIN_SCENE
    )
    base_rollout_rows = []
    ...
    source_paths = {
        ...
        / "validate_relational_teacher_v9_one_scene_overfit_smoke.py",
        "contract": PREPARE_ROOT / "relational_teacher_v9_overfit_smoke_contract.py",
    ...
    report = {
        ...
        "selection_policy": "earliest_eligible_train_only_monitor_else_final_diagnostic",
        "rollout": {
    ...
    print(f"[ONE_SCENE_OVERFIT_{status}] Teacher-v9 all-sittable learnability smoke")
