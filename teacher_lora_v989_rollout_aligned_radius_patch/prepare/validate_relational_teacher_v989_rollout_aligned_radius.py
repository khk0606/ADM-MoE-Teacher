#!/usr/bin/env python3
"""Deep validator for Teacher-v9.8.9 rollout-aligned radius response."""
...
def main() -> None:
    ...
    if value.get("schema") != SCHEMA:
        raise ValueError("Teacher-v9.8.9 report schema changed")
    if (
    ...
    if policy_value != POLICY or canonical_sha256(policy_value) != POLICY_ID:
        raise ValueError("Teacher-v9.8.9 policy changed")
    if value.get("policy_sha256") != hashes["response_policy"]:
        raise ValueError("Teacher-v9.8.9 policy file hash changed")
    ...
    if set(arrays) != required:
        raise ValueError("Teacher-v9.8.9 array inventory changed")
    if (
        ...
        raise ValueError("Teacher-v9.8.9 array shape/order changed")
    for name, array in arrays.items():
        if np.issubdtype(array.dtype, np.number) and not np.isfinite(array).all():
            raise ValueError("Teacher-v9.8.9 array contains NaN/Inf: " + name)
    ...
    expected_checks = {
        ...
        "five_radii_share_exact_step4_start": True,
        "five_radii_produce_distinct_lora_states": True,
    ...
    if (
        ...
        raise ValueError("Teacher-v9.8.9 status/authorization changed")
    expected_binding = canonical_sha256(
        {
            "v988_binding_id": value["v988_binding_id"],
    ...
    if value.get("binding_id") != expected_binding:
        raise ValueError("Teacher-v9.8.9 binding ID changed")
    if list(summary_file.parent.glob("*.pt")) or list(summary_file.parent.glob("*.pth")):
        raise ValueError("Teacher-v9.8.9 contains forbidden model state")

    print("[ROLLOUT_ALIGNED_RADIUS_{}] Teacher-v9.8.9 integrity".format(status))
    print("[PASS] exact v9.8.8 direction and five common-start radii verified")
