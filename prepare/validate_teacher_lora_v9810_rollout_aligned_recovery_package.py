#!/usr/bin/env python3
"""Validate the immutable Teacher-v9.8.10 recovery package."""
...
def main() -> None:
    ...
    if manifest.get("schema") != "teacher_lora_v9810_rollout_aligned_recovery_package_v1":
        raise ValueError("Teacher-v9.8.10 package schema changed")
    files = manifest.get("files")
    if not isinstance(files, dict) or not files:
        raise ValueError("Teacher-v9.8.10 package inventory is absent")
    actual = {
    ...
    if actual != set(files):
        raise ValueError("Teacher-v9.8.10 package inventory changed")
    for name, expected in files.items():
        if sha256_file(root / name) != expected:
            raise ValueError("Teacher-v9.8.10 package file changed: " + name)

    expected_authorization = {
        "sealed_v989_monotone_bed_only_failure_input": True,
        ...
        "actual_two_scene_k3_recovery_bracket": True,
    ...
    if manifest.get("authorization") != expected_authorization:
        raise ValueError("Teacher-v9.8.10 authorization policy changed")

    runbook = (
        root / "TEACHER_V9810_ROLLOUT_ALIGNED_RECOVERY_RUNBOOK.md"
    ...
    runner = root / "prepare/evaluate_relational_teacher_v9810_rollout_aligned_recovery.py"
    ...
    for literal in (
        ...
        if literal not in source:
            raise ValueError("rollout-aligned recovery runner guard changed: " + literal)

    contract = (
        root / "prepare/relational_teacher_v9810_rollout_aligned_recovery_contract.py"
    ).read_text(encoding="utf-8")
    for literal in (
        ...
        if literal not in contract:
            raise ValueError("rollout-aligned recovery contract literal changed: " + literal)

    validator = (
        root / "prepare/validate_relational_teacher_v9810_rollout_aligned_recovery.py"
    ).read_text(encoding="utf-8")
    for literal in (
        "two_scene_preflight_checks(",
    ...
    print("[PACKAGE_PASS] Teacher-v9.8.10 rollout-aligned recovery delivery")
    print("[PASS] {} files and fail-closed authorization verified".format(len(files)))
    print("[PASS] one-block shell, monotone recovery, actual K=3 and no-checkpoint guards")
