#!/usr/bin/env python3
"""Run the selected v9.8.8 direction at five radii on both train scenes."""
...
from relational_teacher_v989_rollout_aligned_radius_contract import (  # noqa: E402
    AUDIT_SCENE,
    DEVELOPMENT_SCENE,
    LORA_ALPHA,
    LORA_RANK,
...
def main() -> None:
    ...
    if not args.device.startswith("cuda:") or not torch.cuda.is_available():
        raise RuntimeError("Teacher-v9.8.9 rollout-aligned radius response requires CUDA")
    if (
        ...
        raise ValueError("Teacher-v9.8.9 protocol is sealed")
    output_dir = args.output_dir.expanduser().resolve()
    if output_dir.exists():
        raise FileExistsError("refusing to overwrite Teacher-v9.8.9 output")
    configure_reproducibility(args.seed)

    v988_file = args.direction_report.expanduser().resolve()
    ...
    policy_file = output_dir / "rollout_aligned_radius_policy.json"
    atomic_write_json(policy_file, POLICY)
    if canonical_sha256(read_json(policy_file)) != POLICY_ID:
        raise AssertionError("rollout-aligned radius policy hash changed")
    ...
    if not np.isfinite(raw_gram).all() or raw_asymmetry > RAW_GRAM_ASYMMETRY_CAP:
        raise RuntimeError("v9.8.9 raw Gram matrix is invalid")
    ...
    arrays_file = output_dir / "rollout_aligned_radius_maps.npz"
    save_arrays = {
    ...
    source_paths = {
        ...
        "validator": PREPARE_ROOT / "validate_relational_teacher_v989_rollout_aligned_radius.py",
        "contract": PREPARE_ROOT / "relational_teacher_v989_rollout_aligned_radius_contract.py",
        "summarizer": PREPARE_ROOT / "summarize_relational_teacher_v989_rollout_aligned_radius.py",
    ...
    top_checks = {
        ...
        "five_radii_share_exact_step4_start": all(
            row["pre_state_sha256"] == selected_state_sha256 for row in response_rows
        ),
        "five_radii_produce_distinct_lora_states": len(set(candidate_state_hashes))
        == len(RADIUS_GRID),
    ...
    report["binding_id"] = canonical_sha256(
        {
            "v988_binding_id": report["v988_binding_id"],
    ...
    _finite_tree(report, "Teacher-v9.8.9 report")
    report_file = output_dir / "summary.json"
    atomic_write_json(report_file, report)
    if list(output_dir.glob("*.pt")) or list(output_dir.glob("*.pth")):
        raise AssertionError("Teacher-v9.8.9 unexpectedly saved model state")
    print("[ROLLOUT_ALIGNED_RADIUS_{}] Teacher-v9.8.9".format(status))
    print("[PASS] exact v9.8.8 selected direction and common step-4 starts verified")
    print("[PASS] five radii x two scenes x K=3 x two prompts completed")
