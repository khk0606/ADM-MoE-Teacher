# Teacher-v9.8.11: disjoint-seed two-scene K=3 replication
...
run_v9811_rollout_aligned_replication() {
  ...
  python prepare/test_relational_teacher_v9811_rollout_aligned_replication_contract.py || return 1
  ...
  V9810=$DATA/experiments/teacher_lora_v9810/rollout_aligned_recovery_s20261026_v1/summary.json
  OUT=$DATA/experiments/teacher_lora_v9811/rollout_aligned_replication_s20261027_v1
  if [ -e "$OUT" ]; then
    echo "[STOP] OUT already exists; preserve it and change only the final version suffix"
    return 1
  fi
  python prepare/validate_relational_teacher_v9810_rollout_aligned_recovery.py --summary "$V9810" || return 1
  export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:128
  python -u prepare/evaluate_relational_teacher_v9811_rollout_aligned_replication.py --selected-recovery-summary "$V9810" --source-dataset-root "$SRC" --dataset-root "$DATA" --index "$DATA/index.json" --v5-dataset-root "$V5DATA" --v5-split "$V5DATA/splits/chair23_bed2_whiteboard12_multistart24_v1.json" --stats-file data/Mean_Std_Cont_HumanML3D_HUMANISE_PROX_contact_cont_joints_0.8_fur.npz --original-checkpoint outputs/CDM-Perceiver-ALL/ckpt/model300000.pt --v5-checkpoint "$V5/fewshot_cdm.pt" --v5-evidence-report "$V5/gate0a_report.json" --output-dir "$OUT" --diffusion-steps 500 --seed 20261016 --lora-rank 4 --lora-alpha 8 --device cuda:0 --no-progress
  RUN_RC=$?
  if [ ! -f "$OUT/summary.json" ]; then
    echo "[STOP] CUDA run did not produce summary.json (exit=$RUN_RC)"
    return 1
  fi
  python prepare/validate_relational_teacher_v9811_rollout_aligned_replication.py --summary "$OUT/summary.json" || return 1
  python prepare/summarize_relational_teacher_v9811_rollout_aligned_replication.py --summary "$OUT/summary.json" || return 1
  return "$RUN_RC"
}
run_v9811_rollout_aligned_replication
V9811_RC=$?
unset -f run_v9811_rollout_aligned_replication
echo "[DONE] Teacher-v9.8.11 block exit=$V9811_RC; this terminal remains open"
```

PASS authorizes only fresh two-scene rollout-aligned multi-update calibration.
FAIL authorizes only cross-scene training-objective redesign. Neither result
