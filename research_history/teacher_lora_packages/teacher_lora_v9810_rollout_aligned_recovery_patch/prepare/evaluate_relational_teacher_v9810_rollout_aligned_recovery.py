#!/usr/bin/env python3
"""Run the selected v9.8.8 direction in the v9.8.10 recovery bracket."""
...
def _validate_v989(path: Path) -> Mapping[str, object]:
    value = read_json(path)
    if (
        value.get("schema") != V989_SCHEMA
        or value.get("status") != "FAIL"
        or value.get("selected_radius") is not None
        or value.get("radius_grid") != list(FAILED_RADIUS_GRID)
        ...
        raise ValueError("sealed Teacher-v9.8.9 failure authority changed")
    validate_failed_recovery_signature(value)
    paths = value.get("paths")
    hashes = value.get("path_sha256")
    if not isinstance(paths, Mapping) or not isinstance(hashes, Mapping) or set(paths) != set(hashes):
        raise ValueError("sealed Teacher-v9.8.9 path binding changed")
    for name, raw in paths.items():
        bound = Path(str(raw)).expanduser().resolve()
        if not bound.is_file() or sha256_file(bound) != hashes[name]:
            raise ValueError("sealed Teacher-v9.8.9 bound file changed: " + str(name))
    if list(path.parent.glob("*.pt")) or list(path.parent.glob("*.pth")):
        raise ValueError("sealed Teacher-v9.8.9 contains forbidden model state")
    return value
...
def main() -> None:
    ...
    if not args.device.startswith("cuda:") or not torch.cuda.is_available():
        raise RuntimeError("Teacher-v9.8.10 rollout-aligned recovery requires CUDA")
    if (
        ...
        raise ValueError("Teacher-v9.8.10 protocol is sealed")
    output_dir = args.output_dir.expanduser().resolve()
    if output_dir.exists():
        raise FileExistsError("refusing to overwrite Teacher-v9.8.10 output")
    configure_reproducibility(args.seed)

    v989_file = args.failed_radius_summary.expanduser().resolve()
    v989 = _validate_v989(v989_file)
    v989_maps_file = Path(str(v989["paths"]["response_maps"])).resolve()
    v989_arrays = _load_npz(v989_maps_file)
    v988_file = Path(str(v989["paths"]["v988_report"])).resolve()
    v988 = _validate_v988(v988_file)
    if v989.get("v988_binding_id") != v988.get("binding_id"):
        raise ValueError("sealed Teacher-v9.8.9 to v9.8.8 binding changed")
    ...
    policy_file = output_dir / "rollout_aligned_recovery_policy.json"
    atomic_write_json(policy_file, POLICY)
    if canonical_sha256(read_json(policy_file)) != POLICY_ID:
        raise AssertionError("rollout-aligned recovery policy hash changed")
    ...
    rollout_cache: Dict[str, Dict[int, list]] = {SOURCE_SCENE: {}, AUDIT_SCENE: {}}
    ...
    if not np.array_equal(base_normalized, v988_geometry["base_normalized"]):
        raise AssertionError("exact Teacher-v9.8.8 K=3 Base states did not reproduce")
    if not np.array_equal(base_normalized, v989_arrays["base_normalized"]):
        raise AssertionError("exact Teacher-v9.8.9 K=3 Base maps changed")
    ...
    for scene in (SOURCE_SCENE, AUDIT_SCENE):
        for generation in range(3):
            states = torch.cat(
                [rollout_cache[scene][generation][prompt][0] for prompt in range(2)],
                dim=0,
            ).to(args.device)
    ...
    if not np.isfinite(raw_gram).all() or raw_asymmetry > RAW_GRAM_ASYMMETRY_CAP:
        raise RuntimeError("v9.8.10 raw Gram matrix is invalid")
    ...
    for radius_index, radius in enumerate(RADIUS_GRID):
        ...
        for scene_index, scene in enumerate((SOURCE_SCENE, AUDIT_SCENE)):
            for generation in range(3):
                for prompt_index, text in enumerate(prompt_texts):
                    ...
                    print(
                        "[RADIUS {:.5f}] scene={} generation={} prompt={}".format(
        ...
        candidate_v5_predictions[radius_index] = candidate_v5.detach().cpu().numpy().astype(np.float32)
        candidate_v5_dense = (
        ...
        print(
            "[RESPONSE R={:.5f}] eligible={} room0101_failed={} room0102_failed={}".format(
                radius,
                row["eligible"],
    ...
    arrays_file = output_dir / "rollout_aligned_recovery_maps.npz"
    save_arrays = {
        "scene_ids": np.asarray((SOURCE_SCENE, AUDIT_SCENE)),
        "prompt_ids": np.asarray(PROMPT_IDS),
        "audit_seed_table": np.asarray(
            [stable_rollout_seeds(generation) for generation in range(3)], np.int64
        ),
    ...
    source_paths = {
        ...
        "validator": PREPARE_ROOT / "validate_relational_teacher_v9810_rollout_aligned_recovery.py",
        "contract": PREPARE_ROOT / "relational_teacher_v9810_rollout_aligned_recovery_contract.py",
        "summarizer": PREPARE_ROOT / "summarize_relational_teacher_v9810_rollout_aligned_recovery.py",
        "v989_summary": v989_file,
        "v989_maps": v989_maps_file,
    ...
    top_checks = {
        ...
        "five_recovery_radii_share_exact_step4_start": all(
            row["pre_state_sha256"] == selected_state_sha256 for row in response_rows
        ),
        "five_recovery_radii_produce_distinct_lora_states": len(set(candidate_state_hashes))
        == len(RADIUS_GRID),
    ...
    report = {
        ...
        "v989_binding_id": v989["binding_id"],
    ...
    report["binding_id"] = canonical_sha256(
        {
            "v989_binding_id": report["v989_binding_id"],
    ...
    _finite_tree(report, "Teacher-v9.8.10 report")
    report_file = output_dir / "summary.json"
    atomic_write_json(report_file, report)
    if list(output_dir.glob("*.pt")) or list(output_dir.glob("*.pth")):
        raise AssertionError("Teacher-v9.8.10 unexpectedly saved model state")
    print("[ROLLOUT_ALIGNED_RECOVERY_{}] Teacher-v9.8.10".format(status))
    print("[PASS] sealed v9.8.9 monotone Bed recovery and exact v9.8.8 direction verified")
    print("[PASS] five recovery radii x two scenes x K=3 x two prompts completed")
