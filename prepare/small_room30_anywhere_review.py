...
from small_room30_anywhere_data import TEXTS,ANYWHERE
from small_room30_anywhere_data import RUN_POLICY as POLICY

PURPOSE_KO = {
    ANYWHERE: '목적 가구 제한 없이 앉기 — history 비교',
...
class ReviewSource:
    def __init__(self, summary, dataset):
        ...
        if self.report.get('status') != 'ANYWHERE_COMPLETE_REVIEW_REQUIRED':
        ...
        if self.manifest.get('schema') != 'small_room30_anywhere_run_v1' or self.manifest.get('policy') != POLICY:
            raise ValueError('Wrong competition manifest')
        if self.manifest.get('prompt_texts') != TEXTS:
            raise ValueError('Prompt text mismatch')
        if sha(self.root/'text_features.npz')!=self.manifest['text_features_sha256']:
            raise ValueError('Encoded text features changed')
        ...
        for name in ('small_room30_student_data.py', 'relation_aware_moe_v2_contract.py', 'small_room30_competition_data.py', 'small_room30_competition_review.py', 'small_room30_anywhere_data.py', 'small_room30_anywhere_review.py'):
