#!/usr/bin/env bash
set -euo pipefail
trap 'RUN_RC=$?; echo "[DONE] Teacher-v10.2 block exit=$RUN_RC; this terminal remains open"' EXIT
...
test -f "$FAILED" || { echo "[STOP] missing Teacher-v10.1 failure: $FAILED"; exit 1; }
