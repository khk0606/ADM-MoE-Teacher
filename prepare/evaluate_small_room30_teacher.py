#!/usr/bin/env python3
"""Frozen Base vs saved research adapter, paired free-noise 500-step rollouts."""
import argparse
import json
import sys
...
def main():
    ...
    p.add_argument('--training-summary', type=Path, default=Path('outputs/small_room30_research_run01/summary.json'))
    ...
    if (training.get('schema') != TRAIN_SCHEMA or training.get('status') != 'RESEARCH_TRAINING_FINISHED_NOT_EVALUATED'
            or training.get('teacher_checkpoint_authorized') is not False):
        raise ValueError('Expected completed research training summary, not smoke')
    cp_info = training['last_checkpoint']
    cp_path = Path(cp_info['path'])
    if sha(cp_path) != cp_info['sha256']: raise ValueError('Research checkpoint hash mismatch')
    current = binding(data, training['binding']['inputs'], REPO)
    if current != training['binding']: raise ValueError('Training code/data/base binding changed; do not bypass')
    import torch
    from small_room30_checkpoint_io import load_checkpoint
    saved = load_checkpoint(cp_path)
    if (saved.get('schema') != TRAIN_SCHEMA or saved.get('purpose') != 'RESEARCH_TRAINING'
    ...
    manifest = dict(schema=SCHEMA, mode=args.mode, cases=cases,
        ...
        evaluation_code_sha256={n: sha(REPO / n) for n in EVAL_FILES},
    ...
    try:
        ...
        runtime = Runtime(data, current['inputs'], args.device, training['seed'])
