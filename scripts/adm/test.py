...
def test(cfg: DictConfig) -> None:
    ...
    # prepare testing dataset # 테스트 데이터 생성
    test_dataset = create_dataset(cfg.task.dataset, 'test', gpu=cfg.gpu, **cfg.task.test)
    logger.info(f'Load test dataset size: {len(test_dataset)}') # 테스트 데이터 몇개인지
    ...
    for i, data in enumerate(test_dataloader):
        ...
        for key in data:
            if key.startswith('c_') or key.startswith('info_'): #c_ : condition 입력값들
        ...
        for k in range(repeat_times):
            if cfg.model.name.startswith('CMDM'):# CMDM 실행 시 사용할 k번째 contact map을 고르는 코드
            ...
            sample = sample_fn( # 실제 샘플링(diffusion) 수행
                model,#CDM, CMDM
                x.shape,
                clip_denoised=False,
                noise=None, # 미리 준비한 noise가 없으므로 diffusion 내부에서 새로운 Gaussian noise를 생성
                model_kwargs=x_kwargs, # cdm->텍스트 + 장면, cmdm->텍스트 + 장면 + contact map + motion mask
                progress=True,
            )
...
def main(cfg: DictConfig) -> None:
    ...
    ## setup random seed
    SEED = cfg.seed
    torch.backends.cudnn.benchmark = False # 같은 seed 사용했을 때 결과를 최대한 동일하게 만들기 위한 설정
    torch.backends.cudnn.deterministic = True
    torch.manual_seed(SEED) # 각 라이브러리의 난수 생성을 같은 seed로 고정
    torch.cuda.manual_seed(SEED)
    torch.cuda.manual_seed_all(SEED)
    random.seed(SEED)
    np.random.seed(SEED)

    ## compute modeling dimension #cmdm이라면 22 * 3 = 66
