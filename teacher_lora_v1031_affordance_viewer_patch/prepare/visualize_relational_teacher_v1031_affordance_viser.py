#!/usr/bin/env python3
"""Read-only six-panel Viser viewer for Teacher-v10.3.1 PASS maps."""
...
from relational_teacher_v1031_affordance_viewer_common import (
    CHANNEL_ORDER,
    affordance_colors,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Read-only Teacher-v10.3.1 PASS affordance-map viewer."
    )
...
def _validate_report(report_file: Path) -> tuple[Mapping[str, object], Path]:
    ...
    if (
        ...
        or report.get("shortlisted_steps") != SHORTLIST
        ...
        or report.get("authorizes_extended_onpolicy_training") is not True
        or report.get("authorizes_checkpoint") is not False
        or not isinstance(rows, list)
        or len(rows) != UPDATE_COUNT
        or [row.get("step") for row in rows] != list(range(1, UPDATE_COUNT + 1))
        or any(row.get("eligible") is not True for row in rows)
        or any(row.get("failed_checks") != [] for row in rows)
    ):
        raise ValueError("viewer rejected the Teacher-v10.3.1 PASS authority")
    paths = report.get("paths")
    hashes = report.get("path_sha256")
    if not isinstance(paths, Mapping) or not isinstance(hashes, Mapping):
        raise ValueError("Teacher-v10.3.1 path binding is absent")
    ...
    if (
        ...
        raise ValueError("Teacher-v10.3.1 calibration map hash changed")
...
def _validate_arrays(arrays: Mapping[str, np.ndarray]) -> None:
    ...
    if not required.issubset(arrays):
        raise ValueError("Teacher-v10.3.1 viewer arrays are incomplete")
    ...
    if (
        ...
        raise ValueError("Teacher-v10.3.1 map shape/order changed")
    for prefix in ("source", "audit"):
        if (
            ...
            != (len(ROLES), POINT_COUNT)
            or arrays[prefix + "_instance_targets"].shape
            != (len(ROLES), POINT_COUNT, 6)
            or arrays[prefix + "_all_target"].shape != (POINT_COUNT, 6)
            or arrays[prefix + "_instance_names"].shape != (len(ROLES),)
        ):
...
def _blend(map_colors: np.ndarray, rgb: np.ndarray, amount: float) -> np.ndarray:
    ...
    return np.rint((1.0 - amount) * map_colors + amount * rgb).clip(
        0, 255
    ).astype(np.uint8)
...
def main() -> None:
    ...
    server = viser.ViserServer(
        host=args.host,
        port=args.port,
        label="Teacher-v10.3.1 PASS affordance maps",
    )
    server.gui.add_markdown(
        "## Teacher-v10.3.1 PASS actual-rollout audit\n"
        "**검증된 두 train 장면의 저장된 500-step K=3 결과만 읽습니다.**  \n"
        "위: `Input RGB → GT → v10.3 Start`  \n"
        "아래: `선택 Update → Update−Start → |Update−GT|`  \n"
        "shortlist는 `1 → 2`이며 기본 표시는 update 1입니다.  \n"
        "연속 면은 표시 전용 XY 보간이고 검증은 원본 8192점을 사용했습니다.  \n"
        "현재 calibration은 **PASS**지만 all-three는 다음 장기 학습의 진단 대상입니다."
    )
    scene_control = server.gui.add_dropdown(
        "Scene", options=SCENES, initial_value=SCENES[0]
    )
    state_options = ("start",) + tuple(
        "update_{}".format(index) for index in range(1, UPDATE_COUNT + 1)
    )
    update_control = server.gui.add_dropdown(
        "State", options=state_options, initial_value="update_1"
    )
    ...
    def render() -> None:
        with lock:
            ...
            prompt = str(prompt_control.value)
            ...
            if focus == "all_verified":
                ...
                gt = np.asarray(arrays[prefix + "_all_target"], np.float32)
            else:
            ...
            if state == "start":
                candidate = start
                row = None
                state_label = "v10.3 Start"
            else:
                step = int(state.split("_")[-1])
                candidate = np.asarray(
                    arrays["candidates"][
                        step - 1, scene_index, generation, prompt_index
                    ],
                    np.float32,
                )
                row = report["monitor_rows"][step - 1]
                state_label = "Update {}".format(step)
            ...
            difference = candidate_scalar - start_scalar
            error = np.abs(candidate_scalar - gt_scalar)
            difference_limit = max(float(np.abs(difference).max()), 1e-8)
            maps = (None, gt_scalar, start_scalar, candidate_scalar, difference, error)
            names = (
                "Input 3D Scene (RGB)",
            ...
            if row is None:
                counts = report["start_all_three_counts"][scene]
                status.content = (
                    "**현재 표시:** `{}` · `v10.3 start` · `generation {}` · "
                    "`{}` · `{}`  \n"
                    "전체 calibration 판정: **PASS** · shortlist: `{}`  \n"
                    "All-three 진단: watch `{}/3`, write `{}/3`"
                ).format(
                    scene,
                    generation,
                    prompt,
                    ROLE_LABELS[focus],
                    report["shortlisted_steps"],
                    counts[PROMPT_IDS[0]],
                    counts[PROMPT_IDS[1]],
                )
            else:
                pooled = row["candidate_pooled"][scene][prompt_id]["instances"]
                names_by_role = arrays[prefix + "_instance_names"].tolist()
                recall = [float(pooled[name]["soft_recall"]) for name in names_by_role]
                mae = [
                    float(pooled[name]["active_support_mae"])
                    for name in names_by_role
                ]
                counts = row["all_three_counts"][scene]
                v5_change = 100.0 * (
                    sum(row["candidate_v5_dense"]) / sum(row["start_v5_dense"])
                    - 1.0
                )
                shortlist_note = (
                    "shortlist #{}".format(SHORTLIST.index(row["step"]) + 1)
                    if row["step"] in SHORTLIST
                    else "eligible, not shortlisted"
                )
                status.content = (
                    "**현재 표시:** `{}` · `update {}` · `generation {}` · "
                    "`{}` · `{}`  \n"
                    "Update eligible: **{}** · `{}` · failed: `{}`  \n"
                    "Pooled recall Bed/Normal/High: `{:.6f} / {:.6f} / {:.6f}`  \n"
                    "Pooled MAE Bed/Normal/High: `{:.6f} / {:.6f} / {:.6f}`  \n"
                    "v5 change: `{:+.3f}%` · All-three 진단: watch `{}/3`, write `{}/3`"
                ).format(
                    scene,
                    row["step"],
                    generation,
    ...
    render()
    print("[VIEWER_PASS] Teacher-v10.3.1 PASS actual-rollout maps loaded read-only")
    print("[PASS] two scenes, v10.3 Start, six updates, GT and map hash verified")
    print("[PASS] default update 1 follows validated shortlist [1, 2]")
