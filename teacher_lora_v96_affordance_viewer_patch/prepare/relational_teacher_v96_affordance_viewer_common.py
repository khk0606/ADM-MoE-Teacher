#!/usr/bin/env python3
"""Pure NumPy helpers for the read-only Teacher-v9.6 map viewer."""
...
OBJECT_ORDER = ("bed_01", "chair_01", "chair_06")
OBJECT_LABELS = {
    "all_verified": "All verified Sit objects",
    "bed_01": "Bed",
    "chair_01": "Normal Chair",
    "chair_06": "High Chair",
}
...
def exact_topk_sets(
    ...
    *,
    ...
    if prediction.shape != target.shape or prediction.ndim != 2 or prediction.shape[1] != 6:
        raise ValueError("top-k prediction/target shape changed")
    if object_mask.shape != (prediction.shape[0],) or not np.any(object_mask):
        raise ValueError("top-k object mask is invalid")
    target_any = target.max(axis=-1)
    prediction_any = prediction.max(axis=-1)
    active = np.flatnonzero(object_mask & (target_any >= float(active_threshold)))
    points = np.flatnonzero(object_mask)
    if active.size == 0:
        raise ValueError("top-k target support is empty")
    count = max(1, int(math.ceil(float(active.size) * float(top_fraction))))
    target_order = np.lexsort((active, -target_any[active]))
    prediction_order = np.lexsort((points, -prediction_any[points]))
    return active[target_order[:count]], points[prediction_order[:count]]


def topk_change_masks(
    base: np.ndarray,
    candidate: np.ndarray,
    target: np.ndarray,
    object_mask: np.ndarray,
) -> Dict[str, np.ndarray]:
    target_top, base_top = exact_topk_sets(base, target, object_mask)
    target_top_again, candidate_top = exact_topk_sets(candidate, target, object_mask)
    if not np.array_equal(target_top, target_top_again):
        raise AssertionError("target top-k set changed")
    left = np.zeros(object_mask.shape, dtype=bool)
    entered = np.zeros(object_mask.shape, dtype=bool)
    left[np.setdiff1d(base_top, candidate_top)] = True
    entered[np.setdiff1d(candidate_top, base_top)] = True
    return {
        "target_top": np.isin(np.arange(object_mask.size), target_top),
        "base_top": np.isin(np.arange(object_mask.size), base_top),
        "candidate_top": np.isin(np.arange(object_mask.size), candidate_top),
        "left": left,
        "entered": entered,
    }
