#!/usr/bin/env python3
"""Fresh, train-only Teacher-v9.3 exact top-k response grid.
...
from relational_teacher_v93_exact_topk_contract import (  # noqa: E402
    CANDIDATES,
    GRAD_CLIP,
    SCHEMA,
    SEED,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--failed-v92-report", type=Path, required=True)
...
def _validate_failed_v92(path: Path) -> Mapping[str, object]:
    value = read_json(path)
    if (
        value.get("schema") != V92_SCHEMA
        or value.get("status") != "FAIL"
        or value.get("train_scene") != TRAIN_SCENE
        or value.get("steps_per_candidate") != 6
        ...
        != ["at_least_one_candidate_admissible"]
        or value.get("authorizes_early_rollout_canary") is not False
    ):
        raise ValueError("Teacher-v9.2 hotspot/trust failure binding changed")
    paths = value.get("paths")
    hashes = value.get("path_sha256")
    if not isinstance(paths, Mapping) or not isinstance(hashes, Mapping):
        raise ValueError("v9.2 failure path binding is absent")
    if set(paths) != set(hashes):
        raise ValueError("v9.2 failure path/hash inventory differs")
    for name, raw in paths.items():
        source = Path(str(raw)).expanduser().resolve()
        if not source.is_file() or sha256_file(source) != hashes[name]:
            raise ValueError("v9.2 failure file changed: " + str(name))
    if list(path.parent.glob("*.pt")) or list(path.parent.glob("*.pth")):
        raise ValueError("v9.2 failure contains forbidden model state")
    rows = value.get("candidates")
    if not isinstance(rows, list) or len(rows) != 3:
        raise ValueError("v9.2 failure candidate rows changed")
    if len({str(row.get("prediction_sha256")) for row in rows}) != 3:
        raise ValueError("v9.2 candidate outputs are not independently distinct")
    for row in rows:
        if row.get("eligible") is not False or not {
            "bed_mean_topk_improves",
            "high_chair_mean_topk_improves",
        }.issubset(set(row.get("failed_checks", []))):
            raise ValueError("v9.2 per-object failure diagnosis changed")
    return value
...
def _objective_values(
    ...
    overlap_rows = [
        float(item)
        for item in objective["per_instance_exact_topk_overlap"].mean(dim=0).tolist()
    ]
    metric_overlaps = [
        float(np.mean([row["instances"][name]["topk_overlap"] for row in metrics]))
        for name in ("bed_01", "chair_01", "chair_06")
    ]
    if any(abs(one - two) > 2e-7 for one, two in zip(overlap_rows, metric_overlaps)):
    ...
    return {
        ...
        "exact_topk_swap_macro": float(objective["exact_topk_swap_macro"].item()),
        "per_instance_active_support": [
            float(item)
            for item in objective["per_instance_active_support"].mean(dim=0).tolist()
        ],
        "per_instance_exact_topk_swap": [
            float(item)
            for item in objective["per_instance_exact_topk_swap"].mean(dim=0).tolist()
        ],
...
def main() -> None:
    ...
    if not args.device.startswith("cuda:") or not torch.cuda.is_available():
        raise RuntimeError("Teacher-v9.3 exact top-k response requires CUDA")
    if args.diffusion_steps != 500 or args.seed != SEED:
        raise ValueError("Teacher-v9.3 exact top-k protocol is sealed")
    output_dir = args.output_dir.expanduser().resolve()
    if output_dir.exists():
        raise FileExistsError("refusing to overwrite Teacher-v9.3 response grid")
    configure_reproducibility(args.seed)

    failed_v92_file = args.failed_v92_report.expanduser().resolve()
    failed_v92 = _validate_failed_v92(failed_v92_file)
    preflight_file = args.preflight_report.expanduser().resolve()
    preflight, policy = _validate_preflight(preflight_file)
    if failed_v92.get("preflight_binding_id") != preflight.get("binding_id"):
        raise ValueError("v9.2 failure/preflight binding differs")
    ...
    before_objective = corrected_v93_objective(
    ...
    saved_normalized = []
    saved_v5 = []
    timestep_values = TRAINING_TIMESTEPS
    for candidate in CANDIDATES:
        ...
        for step, timestep_value in enumerate(timestep_values, start=1):
            ...
            objective = corrected_v93_objective(
            ...
            total = (
                float(candidate["instance_primary_weight"]) * objective["instance_macro_primary"]
                + float(candidate["verified_union_weight"]) * objective["verified_union"]
                + float(candidate["environment_weight"]) * objective["environment_auxiliary"]
                + float(candidate["active_support_weight"]) * objective["active_support_macro"]
                + float(candidate["exact_topk_swap_weight"]) * objective["exact_topk_swap_macro"]
            ...
            logs.append(
                {
                    ...
                    "exact_topk_swap": float(objective["exact_topk_swap_macro"].detach().item()),
        ...
        print(
            f"[EXACT-TOPK {candidate['name']}] eligible={row['eligible']} "
    ...
    paths = {
        "failed_v92_report": failed_v92_file,
        "failed_v92_maps": Path(str(failed_v92["paths"]["response_maps"])).resolve(),
        ...
        "objective": PREPARE_ROOT / "relational_teacher_v93_exact_topk_objective.py",
        "contract": PREPARE_ROOT / "relational_teacher_v93_exact_topk_contract.py",
        "runner": Path(__file__).resolve(),
        "validator": PREPARE_ROOT / "validate_relational_teacher_v93_exact_topk.py",
    ...
    report = {
        ...
        "learning_rate_grid": [float(row["learning_rate"]) for row in CANDIDATES],
        "grad_clip": GRAD_CLIP,
        ...
        "authorizes_exact_topk_rollout_canary": status == "PASS",
    ...
    report["binding_id"] = canonical_sha256(
        {
            ...
            "failed_v92_binding_id": report["failed_v92_binding_id"],
            "candidate_grid": report["candidate_grid"],
            "selected_candidate": selected_name,
            "response_maps_sha256": path_hashes["response_maps"],
        }
    )
    _finite_tree(report, "exact top-k report")
    report_file = output_dir / "preflight.json"
    atomic_write_json(report_file, report)
    print(f"[EXACT_TOPK_{status}] Teacher-v9.3 six-update response gate")
