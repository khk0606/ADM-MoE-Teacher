#!/usr/bin/env python3
"""CPU contract for Teacher-v10 Viser rendering helpers."""
...
def main() -> None:
    ...
    assert abs(float(ramp[-1].mean() - ramp[0].mean())) < 1e-5
    rgba = rgba_heatmap(affordance_colors(constant), 0.8)
    assert rgba.shape == (32, 32, 4)
    assert np.all(rgba[..., 3] == 204)
    print("[PASS] Teacher-v10 affordance viewer CPU contract")
