...
from small_room30_targeted_v5 import POLICY,source_contract,check_ready,plan


def load_runtime(data,source,bound,device):
    from small_room30_checkpoint_io import load_checkpoint
    from small_room30_targeted_v5_runtime import RepairRuntime
    ...
    if (saved.get('schema')!=SCHEMA or saved.get('purpose')!='RESEARCH_TARGETED_V4_PILOT'
            or saved.get('binding')!=bound['start_binding'] or saved.get('step')!=1570
            or saved.get('seed')!=source['seed'] or saved.get('teacher_checkpoint_authorized') is not False):
        raise ValueError('Reviewed v4 saved1570 checkpoint required')
...
from small_room30_targeted_v5_runtime import check_missing_gradients as check_transitions
...
def main():
    ...
    p.add_argument('--source-summary',type=Path,default=Path('outputs/small_room30_targeted_v4_run01/summary.json'))
    ...
    a=p.parse_args()
    if a.mode=='train' and (not a.readiness or not a.allow_research_training):p.error('Need v5 readiness and research flag')
    ...
    try:
        ...
        from small_room30_targeted_v5_runtime import measured_canary,baseline,acceptance,transactional_step,tree_equal
        ...
        reference_path=Path(bound['reference_summary'])
        ...
        if a.mode=='smoke':
            ...
            measured_canary(rt,bound,reference_path,a.output_dir/'source_canary',reproduce=True)
            with torch.no_grad():
            ...
            row=rt.gradients(optimizer,entries[0],1571,reference_path)
            ...
            def reject(number,scale):
                cp=save('SMOKE_ATTEMPT_%d.pt'%number,1571,'DISPOSABLE_TARGETED_V5_SMOKE')
            ...
            report=dict(common,status='TARGETED_V5_RUNTIME_READY',main_training_started=False,
            ...
            # Reproduce and snapshot1570 inside the run; portable viewer needs no external v4 maps.
            previous=measured_canary(rt,bound,reference_path,a.output_dir/'source_canary',reproduce=True)
            checkpoints=[];decisions=[];stopped=False
            with (a.output_dir/'train.jsonl').open('x') as log:
                for n,entry in enumerate(entries,1):
                    current=next(r for r in previous['rows'] if r['case']==entry['case'])
                    if current['repair']['optimization_complete']:
                        print('[SKIP] Target reached50% endpoint proxy: '+entry['case']['filename'],flush=True)
                        continue
                    print('[V5 %d/16] repair=%s; presence-only preservation'%(n,entry['case']['filename']),flush=True)
                    next_step=1570+len(checkpoints)+1
                    row=rt.gradients(optimizer,entry,next_step,reference_path)
                    def evaluate(number,scale):
                        out=a.output_dir/('proposal_%02d_try_%d'%(n,number))
                        result=measured_canary(rt,bound,reference_path,out)
                    ...
                    cp=save('targeted_v5_adapter_step_%06d.pt'%next_step,next_step,'RESEARCH_TARGETED_V5_PILOT')
                    ...
                    if previous['decision']['optimization_complete']:break
            report=dict(common,status='TARGETED_V5_STOPPED_REJECTED' if stopped else 'TARGETED_V5_FINISHED_REVIEW_REQUIRED',
                completed_step=checkpoints[-1]['step'] if checkpoints else 1570,last_checkpoint=checkpoints[-1] if checkpoints else None,
                ...
                presence_scope='2 exact paths, 14 preservation paths; shared LoRA, not isolated room weights')
        ...
        print('[STOP] Visual review still required even when endpoint proxy completes. No automatic extension or Teacher approval. Send summary and proposal receipts for review.',flush=True)
