#!/usr/bin/env python3
"""Coverage adapter evaluator; saved-array format remains compatible with the unchanged viewer."""
import argparse
import json
import sys
...
def main():
    ...
    p.add_argument('--training-summary', type=Path, default=Path('outputs/small_room30_coverage_run01/summary.json'))
    ...
    from small_room30_coverage import validate_experiment, FILES
    training, source, current = validate_experiment(data, args.training_summary, REPO)
    ...
    if (saved.get('schema') != TRAIN_SCHEMA or saved.get('purpose') != 'RESEARCH_COVERAGE_TRAINING'
