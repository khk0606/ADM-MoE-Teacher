#!/usr/bin/env python3
"""Deep validator for the Teacher-v9.3 exact top-k response grid."""
...
from preflight_relational_teacher_v93_exact_topk import (  # noqa: E402
    _objective_values,
    _validate_failed_v92,
)
...
def _close(left: object, right: object, label: str) -> None:
    ...
    if (
        ...
        # Saved tensors are exact-hash bound.  Only derived CPU-versus-CUDA
        # reductions need a small numerical tolerance.
        if not math.isclose(
            float(left), float(right), rel_tol=2e-6, abs_tol=2e-5
        ):
...
def main() -> None:
    ...
    if value.get("schema") != SCHEMA or value.get("status") not in {"PASS", "FAIL"}:
        raise ValueError("Teacher-v9.3 exact top-k schema/status changed")
    if (
        value.get("seed") != SEED
        or value.get("diffusion_steps") != 500
        or value.get("steps_per_candidate") != STEPS_PER_CANDIDATE
        or value.get("training_timesteps") != list(TRAINING_TIMESTEPS)
        or value.get("monitor_timestep") != 125
        or value.get("learning_rate_grid")
        != [float(row["learning_rate"]) for row in CANDIDATES]
        ...
        or value.get("failed_v92_checkpoint_loaded") is not False
        ...
        raise ValueError("Teacher-v9.3 exact top-k protocol changed")
    lora = value.get("lora")
    ...
    failure = _validate_failed_v92(Path(str(paths["failed_v92_report"])))
    preflight, _ = _validate_preflight(Path(str(paths["preflight_report"])))
    if (
        value.get("failed_v92_binding_id") != failure.get("binding_id")
        or value.get("preflight_binding_id") != preflight.get("binding_id")
    ):
        raise ValueError("exact top-k authority binding changed")
    ...
    with np.load(Path(str(paths["failed_v92_maps"])), allow_pickle=False) as source:
        for name in (
            "xyz",
            "verified_object_mask",
            "unknown_sittable_mask",
    ...
    for index, (specification, row) in enumerate(zip(CANDIDATES, rows)):
        ...
        if row.get("steps") != STEPS_PER_CANDIDATE or row.get("learning_rate") != specification["learning_rate"]:
        ...
        if [item.get("timestep") for item in logs] != list(TRAINING_TIMESTEPS):
    ...
    if (
        ...
        or value.get("authorizes_exact_topk_rollout_canary") is not (status == "PASS")
        or value.get("checks") != expected_top_checks
    ):
    ...
    expected_binding = canonical_sha256(
        {
            ...
            "failed_v92_binding_id": value["failed_v92_binding_id"],
    ...
    _finite_tree(value, "exact top-k report")
    print(f"[EXACT_TOPK_{status}] Teacher-v9.3 response integrity")
    print("[PASS] sealed v9.2 failure and fresh v5r4 authority verified")
    print("[PASS] evaluation-aligned per-object top-k swaps and trust gates recomputed")
