# Teacher-v9.8.9: actual two-scene K=3 rollout-aligned radius response
...
run_v989_rollout_aligned_radius() {
  ...
  python prepare/test_relational_teacher_v989_rollout_aligned_radius_contract.py || return 1
  ...
  V988=$DATA/experiments/teacher_lora_v988/rollout_aligned_direction_s20261024_v1/preflight.json
  OUT=$DATA/experiments/teacher_lora_v989/rollout_aligned_radius_s20261025_v1
  if [ -e "$OUT" ]; then
    echo "[STOP] OUT already exists; preserve it and change only the final version suffix"
    return 1
  fi
  python prepare/validate_relational_teacher_v988_rollout_aligned_direction.py --report "$V988" || return 1
  export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:128
  python -u prepare/evaluate_relational_teacher_v989_rollout_aligned_radius.py --direction-report "$V988" --source-dataset-root "$SRC" --dataset-root "$DATA" --index "$DATA/index.json" --v5-dataset-root "$V5DATA" --v5-split "$V5DATA/splits/chair23_bed2_whiteboard12_multistart24_v1.json" --stats-file data/Mean_Std_Cont_HumanML3D_HUMANISE_PROX_contact_cont_joints_0.8_fur.npz --original-checkpoint outputs/CDM-Perceiver-ALL/ckpt/model300000.pt --v5-checkpoint "$V5/fewshot_cdm.pt" --v5-evidence-report "$V5/gate0a_report.json" --output-dir "$OUT" --diffusion-steps 500 --seed 20261016 --lora-rank 4 --lora-alpha 8 --device cuda:0 --no-progress
  RUN_RC=$?
  if [ ! -f "$OUT/summary.json" ]; then
    echo "[STOP] CUDA run did not produce summary.json (exit=$RUN_RC)"
    return 1
  fi
  python prepare/validate_relational_teacher_v989_rollout_aligned_radius.py --summary "$OUT/summary.json" || return 1
  python prepare/summarize_relational_teacher_v989_rollout_aligned_radius.py --summary "$OUT/summary.json" || return 1
  return "$RUN_RC"
}
run_v989_rollout_aligned_radius
V989_RC=$?
unset -f run_v989_rollout_aligned_radius
echo "[DONE] Teacher-v9.8.9 block exit=$V989_RC; this terminal remains open"
```

PASS authorizes only fresh two-scene rollout-aligned calibration. FAIL
authorizes only objective/inference redesign. Neither result authorizes a
