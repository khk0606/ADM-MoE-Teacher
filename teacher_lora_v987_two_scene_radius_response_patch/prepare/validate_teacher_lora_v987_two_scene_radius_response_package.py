#!/usr/bin/env python3
"""Validate the immutable Teacher-v9.8.7 radius-response package."""
...
def main() -> None:
    ...
    if manifest.get("schema") != "teacher_lora_v987_two_scene_radius_response_package_v1":
        raise ValueError("Teacher-v9.8.7 package schema changed")
    files = manifest.get("files")
    if not isinstance(files, dict) or not files:
        raise ValueError("Teacher-v9.8.7 package inventory is absent")
    actual = {
    ...
    if actual != set(files):
        raise ValueError("Teacher-v9.8.7 package inventory changed")
    for name, expected in files.items():
        if sha256_file(root / name) != expected:
            raise ValueError("Teacher-v9.8.7 package file changed: " + name)

    expected_authorization = {
        "sealed_v986_pass_input": True,
    ...
    if manifest.get("authorization") != expected_authorization:
        raise ValueError("Teacher-v9.8.7 authorization policy changed")

    runbook = (
        root / "TEACHER_V987_TWO_SCENE_RADIUS_RESPONSE_RUNBOOK.md"
    ...
    runner = root / "prepare/evaluate_relational_teacher_v987_two_scene_radius_response.py"
    ...
    for literal in (
        ...
        if literal not in source:
            raise ValueError("radius-response runner guard changed: " + literal)

    contract = (
        root / "prepare/relational_teacher_v987_two_scene_radius_response_contract.py"
    ).read_text(encoding="utf-8")
    for literal in (
        ...
        if literal not in contract:
            raise ValueError("radius-response contract literal changed: " + literal)

    validator = (
        root / "prepare/validate_relational_teacher_v987_two_scene_radius_response.py"
    ).read_text(encoding="utf-8")
    for literal in (
        "two_scene_preflight_checks(",
    ...
    print("[PACKAGE_PASS] Teacher-v9.8.7 two-scene radius-response delivery")
    print("[PASS] {} files and fail-closed authorization verified".format(len(files)))
    print("[PASS] one-block shell, common-start grid and no-checkpoint guards")
