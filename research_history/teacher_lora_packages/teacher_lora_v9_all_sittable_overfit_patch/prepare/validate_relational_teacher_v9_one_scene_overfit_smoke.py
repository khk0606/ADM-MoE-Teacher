#!/usr/bin/env python3
"""Deep integrity validator for the Teacher-v9 one-scene overfit smoke."""
...
def main() -> None:
    ...
    if value.get("schema") != SCHEMA or value.get("status") not in {"PASS", "FAIL"}:
        raise ValueError("Teacher-v9 overfit smoke schema/status changed")
    if (
        value.get("seed") != SEED
        ...
        raise ValueError("Teacher-v9 overfit protocol changed")
    if value.get("selection_policy") != (
        "earliest_eligible_train_only_monitor_else_final_diagnostic"
    ):
    ...
    expected_initial, expected_reverse = _stable_seeds(
        SEED, "one_scene_overfit_rollout", TRAIN_SCENE
    )
    ...
    expected_selected = eligible_steps[0] if eligible_steps else STEPS
    ...
    if value.get("authorizes_fresh_response3") is not (expected_status == "PASS"):
    ...
    print(f"[ONE_SCENE_OVERFIT_{expected_status}] Teacher-v9 smoke integrity")
    print("[PASS] fresh v5r4, sealed policy and exact room_0101 binding verified")
