#!/usr/bin/env python3
"""Run the selected v9.8.6 direction at four radii on both train scenes."""
...
from preflight_relational_teacher_v986_cross_scene_direction import (  # noqa: E402
    _capture_design_pair,
    _scene_task_gradients,
)
...
from relational_teacher_v986_cross_scene_direction_contract import (  # noqa: E402
    CANDIDATE_NAMES as V986_CANDIDATE_NAMES,
    POLICY_ID as V986_POLICY_ID,
    RAW_GRAM_ASYMMETRY_CAP,
    TASK_ORDER as V986_TASK_ORDER,
    diagnose_directions,
)
...
def _validate_v986(path: Path) -> Mapping[str, object]:
    value = read_json(path)
    if (
        value.get("schema") != V986_SCHEMA
        or value.get("status") != "PASS"
        or value.get("policy_id") != V986_POLICY_ID
        ...
        or value.get("authorizes_actual_two_scene_k3_direction_response_grid") is not True
        ...
        raise ValueError("sealed Teacher-v9.8.6 PASS authority changed")
    selected = [
        row for row in value.get("direction_candidates", [])
        if row.get("name") == SELECTED_DIRECTION
    ]
    if len(selected) != 1 or selected[0].get("eligible") is not True or selected[0].get(
        "failed_checks"
    ):
        raise ValueError("sealed Teacher-v9.8.6 selected direction changed")
    paths = value.get("paths")
    hashes = value.get("path_sha256")
    if not isinstance(paths, Mapping) or not isinstance(hashes, Mapping) or set(paths) != set(hashes):
        raise ValueError("sealed Teacher-v9.8.6 path binding changed")
    for name, raw in paths.items():
        bound = Path(str(raw)).expanduser().resolve()
        if not bound.is_file() or sha256_file(bound) != hashes[name]:
            raise ValueError("sealed Teacher-v9.8.6 bound file changed: " + str(name))
    if list(path.parent.glob("*.pt")) or list(path.parent.glob("*.pth")):
        raise ValueError("sealed Teacher-v9.8.6 contains forbidden model state")
    return value


def _restore_state(
    named_lora: Mapping[str, torch.nn.Parameter], state: Mapping[str, torch.Tensor]
) -> None:
    if set(named_lora) != set(state):
        raise ValueError("in-memory LoRA state inventory changed")
    with torch.no_grad():
        for name, parameter in named_lora.items():
            parameter.copy_(state[name].to(parameter.device, parameter.dtype))


def _close_array(left: object, right: object, label: str) -> None:
    if not np.allclose(
        ...
        raise AssertionError(label + " differs from sealed Teacher-v9.8.6")


def main() -> None:
    args = parse_args()
    if not args.device.startswith("cuda:") or not torch.cuda.is_available():
        raise RuntimeError("Teacher-v9.8.7 two-scene radius response requires CUDA")
    if (
        ...
        raise ValueError("Teacher-v9.8.7 protocol is sealed")
    output_dir = args.output_dir.expanduser().resolve()
    if output_dir.exists():
        raise FileExistsError("refusing to overwrite Teacher-v9.8.7 output")
    configure_reproducibility(args.seed)

    v986_file = args.direction_report.expanduser().resolve()
    v986 = _validate_v986(v986_file)
    v985_file = Path(str(v986["paths"]["v985_report"])).resolve()
    v985 = read_json(v985_file)
    if (
        v985.get("status") != "FAIL"
        or v985.get("policy_id") != V985_POLICY_ID
        or v985.get("binding_id") != v986.get("v985_binding_id")
    ):
        raise ValueError("sealed Teacher-v9.8.5 failure changed")
    v985_maps_file = Path(str(v985["paths"]["audit_maps"])).resolve()
    v985_arrays = _load_npz(v985_maps_file)
    v984_file = Path(str(v986["paths"]["v984_summary"])).resolve()
    ...
    if (
        ...
        or v984.get("binding_id") != v986.get("v984_binding_id")
    ):
        raise ValueError("sealed Teacher-v9.8.4 authority changed")
    v984_maps_file = Path(str(v986["paths"]["v984_maps"])).resolve()
    v984_arrays = _load_npz(v984_maps_file)
    v98_file = Path(str(v986["paths"]["v98_report"])).resolve()
    v98 = read_json(v98_file)
    v98_maps_file = Path(str(v986["paths"]["v98_maps"])).resolve()
    v98_arrays = _load_npz(v98_maps_file)
    v986_geometry_file = Path(str(v986["paths"]["gradient_geometry"])).resolve()
    v986_geometry = _load_npz(v986_geometry_file)

    source_root = args.source_dataset_root.expanduser().resolve()
    dataset_root = args.dataset_root.expanduser().resolve()
    v5_root = args.v5_dataset_root.expanduser().resolve()
    index_file = _same_bound_arg(args.index, v986, "dataset_index")
    split_file = _same_bound_arg(args.v5_split, v986, "v5_split")
    stats_file = _same_bound_arg(args.stats_file, v986, "stats_file")
    original_checkpoint = _same_bound_arg(args.original_checkpoint, v986, "original_checkpoint")
    v5_checkpoint = _same_bound_arg(args.v5_checkpoint, v986, "v5_checkpoint")
    evidence_file = _same_bound_arg(args.v5_evidence_report, v986, "v5_evidence_report")
    source_index = Path(str(v986["paths"]["source_dataset_index"])).resolve()
    if dataset_root != index_file.parent or source_root != source_index.parent:
        raise ValueError("dataset roots differ from sealed Teacher-v9.8.6")
    if v5_root != v5_checkpoint.parents[2]:
        raise ValueError("v5 dataset root differs from sealed checkpoint layout")

    output_dir.mkdir(parents=True)
    policy_file = output_dir / "two_scene_radius_response_policy.json"
    atomic_write_json(policy_file, POLICY)
    if canonical_sha256(read_json(policy_file)) != POLICY_ID:
        raise AssertionError("two-scene radius policy hash changed")
    ...
    metric_policy = read_json(Path(str(v986["paths"]["metric_policy"])).resolve())
    ...
    rollout_cache: Dict[str, Dict[int, list]] = {
        SOURCE_SCENE: {},
        AUDIT_SCENE: {},
    }
    for scene_index, scene in enumerate((SOURCE_SCENE, AUDIT_SCENE)):
        for generation in range(3):
            ...
            rollout_cache[scene][generation] = []
            for prompt_index, text in enumerate(prompt_texts):
                ...
                rollout_cache[scene][generation].append(
                    (states[SELECTED_TIMESTEP], rng_states[SELECTED_TIMESTEP])
                )
                print(
                    "[BASE] scene={} generation={} prompt={}".format(
                        scene, generation, PROMPT_IDS[prompt_index]
                    ),
                    flush=True,
                )
    if not np.array_equal(base_normalized[0], v984_arrays["base_normalized"]):
        raise AssertionError("room_0101 frozen Base K=3 changed")
    if not np.array_equal(base_normalized[1], v985_arrays["base_normalized"]):
        raise AssertionError("room_0102 frozen Base K=3 changed")
    ...
    diagnosis_seeds = {}
    for scene in (SOURCE_SCENE, AUDIT_SCENE):
        diagnosis_states[scene], diagnosis_seeds[scene] = _capture_design_pair(
            model, diffusion, bundles[scene], args.device, not args.no_progress
        )
    ...
    if selected_state_sha256 != v986["selected_state_sha256"]:
        raise AssertionError("reconstructed step-4 state differs from v9.8.6")
    step4_state = {
    ...
    source_gradients, source_losses = _scene_task_gradients(
        model=model,
        diffusion=diffusion,
        states=diagnosis_states[SOURCE_SCENE].to(args.device),
        kwargs=kwargs[SOURCE_SCENE],
        batch=batches[SOURCE_SCENE],
    ...
    audit_gradients, audit_losses = _scene_task_gradients(
        model=model,
        diffusion=diffusion,
        states=diagnosis_states[AUDIT_SCENE].to(args.device),
        kwargs=kwargs[AUDIT_SCENE],
        batch=batches[AUDIT_SCENE],
        mean_tensor=mean_tensor,
        std_tensor=std_tensor,
        parameters=parameters,
    )
    ...
    if gradients.shape[0] != len(V986_TASK_ORDER):
    ...
    if not np.isfinite(raw_gram).all() or raw_asymmetry > RAW_GRAM_ASYMMETRY_CAP:
        raise RuntimeError("v9.8.7 raw Gram matrix is invalid")
    gram = 0.5 * (raw_gram + raw_gram.T)
    task_losses = source_losses + audit_losses + v5_losses
    _close_array(task_losses, v986_geometry["task_losses"], "task losses")
    _close_array(gram, v986_geometry["gram"], "Gram matrix")
    diagnostic_rows = diagnose_directions(gram)
    if [row["name"] for row in diagnostic_rows] != list(V986_CANDIDATE_NAMES):
        raise AssertionError("v9.8.6 candidate order changed")
    ...
    if len(selected_rows) != 1 or selected_rows[0]["eligible"] is not True:
        raise AssertionError("selected v9.8.6 direction did not reproduce")
    ...
    if not math.isclose(selected_direction_l2, 1.0, rel_tol=2e-5, abs_tol=2e-6):
        raise AssertionError("selected v9.8.6 direction is not unit length")
    ...
    if selected_direction_sha256 != sealed_selected["direction_sha256"]:
        raise AssertionError("selected v9.8.6 direction hash changed")
    if _state_sha256(named_lora) != selected_state_sha256:
        raise AssertionError("gradient diagnosis mutated the step-4 state")
    ...
    candidates_normalized = np.empty((len(RADIUS_GRID), 2, 3, 2, 8192, 6), np.float32)
    candidates_physical = np.empty_like(candidates_normalized)
    candidate_v5_predictions = np.empty((len(RADIUS_GRID), 3, 8192, 6), np.float32)
    response_rows = []
    for radius_index, radius in enumerate(RADIUS_GRID):
        _restore_state(named_lora, step4_state)
        if _state_sha256(named_lora) != selected_state_sha256:
            raise AssertionError("radius candidate did not start from exact step 4")
        apply_flat_direction(parameters, selected_direction.to(args.device), radius)
        candidate_state_sha256 = _state_sha256(named_lora)
        model.eval()
        for scene_index, scene in enumerate((SOURCE_SCENE, AUDIT_SCENE)):
            for generation in range(3):
                for prompt_index, text in enumerate(prompt_texts):
                    state, rng_state = rollout_cache[scene][generation][prompt_index]
                    ...
                    candidates_normalized[
                        radius_index, scene_index, generation, prompt_index
                    ] = final.numpy()
                    print(
                        "[RADIUS {:.4f}] scene={} generation={} prompt={}".format(
                            radius, scene, generation, PROMPT_IDS[prompt_index]
                        ),
                        flush=True,
                    )
        with torch.no_grad():
        ...
        candidate_v5_predictions[radius_index] = (
            candidate_v5.detach().cpu().numpy().astype(np.float32)
        )
        candidate_v5_dense = (
            (candidate_v5 - v5_batch["x"]).square().mean(dim=(1, 2)).cpu().tolist()
        )
        candidates_physical[radius_index] = np.clip(
            candidates_normalized[radius_index] * std.reshape(1, 1, 1, 1, 6)
            + mean.reshape(1, 1, 1, 1, 6),
            0.0,
            1.0,
        ).astype(np.float32)
        ...
        for scene_index, scene in enumerate((SOURCE_SCENE, AUDIT_SCENE)):
            ...
            candidate_scene = candidates_physical[radius_index, scene_index]
        ...
        checks = radius_response_checks(
            ...
            candidate_state_changed=candidate_state_sha256 != selected_state_sha256,
        )
        row = {
            "radius": radius,
        ...
        print(
            "[RESPONSE R={:.4f}] eligible={} room0101_failed={} room0102_failed={}".format(
                radius,
                row["eligible"],
                len(row["scene_failed_checks"][SOURCE_SCENE]),
                len(row["scene_failed_checks"][AUDIT_SCENE]),
            ),
            flush=True,
        )
        del candidate_v5
    ...
    _restore_state(named_lora, step4_state)
    if _state_sha256(named_lora) != selected_state_sha256:
        raise AssertionError("final in-memory state was not restored to sealed step 4")
    ...
    if len(set(candidate_state_hashes)) != len(RADIUS_GRID):
        raise AssertionError("distinct radii produced duplicate LoRA states")
    selection_order = rank_eligible_radii(response_rows)
    selected_radius = selection_order[0] if selection_order else None

    arrays_file = output_dir / "two_scene_radius_response_maps.npz"
    save_arrays = {
        ...
        "radius_grid": np.asarray(RADIUS_GRID, np.float64),
    ...
    source_paths = {
        ...
        "validator": PREPARE_ROOT / "validate_relational_teacher_v987_two_scene_radius_response.py",
        "contract": PREPARE_ROOT / "relational_teacher_v987_two_scene_radius_response_contract.py",
        "summarizer": PREPARE_ROOT / "summarize_relational_teacher_v987_two_scene_radius_response.py",
        "v986_report": v986_file,
        "v986_geometry": v986_geometry_file,
        "v985_report": v985_file,
        "v985_maps": v985_maps_file,
        "v984_summary": v984_file,
        "v984_maps": v984_maps_file,
        "v98_report": v98_file,
        "v98_maps": v98_maps_file,
        "metric_policy": Path(str(v986["paths"]["metric_policy"])).resolve(),
    ...
    top_checks = {
        ...
        "four_radii_share_exact_step4_start": all(
            row["pre_state_sha256"] == selected_state_sha256 for row in response_rows
        ),
        "four_radii_produce_distinct_lora_states": len(set(candidate_state_hashes))
        == len(RADIUS_GRID),
        "both_scene_actual_k3_two_prompt_responses_completed": len(response_rows)
        == len(RADIUS_GRID),
    ...
    report = {
        ...
        "v986_binding_id": v986["binding_id"],
    ...
    report["binding_id"] = canonical_sha256(
        {
            "v986_binding_id": report["v986_binding_id"],
            ...
            "selected_radius": report["selected_radius"],
            "status": report["status"],
        }
    )
    _finite_tree(report, "Teacher-v9.8.7 report")
    report_file = output_dir / "summary.json"
    atomic_write_json(report_file, report)
    if list(output_dir.glob("*.pt")) or list(output_dir.glob("*.pth")):
        raise AssertionError("Teacher-v9.8.7 unexpectedly saved model state")
    print("[TWO_SCENE_RADIUS_RESPONSE_{}] Teacher-v9.8.7".format(status))
    print("[PASS] exact v9.8.6 selected direction and common step-4 starts verified")
    print("[PASS] four radii x two scenes x K=3 x two prompts completed")
