#!/usr/bin/env python3
"""Visually accepted v5 saved1578 full evaluator; unchanged sampler and Viser array format."""
import time
import argparse
import json
import sys
...
def main():
    ...
    p.add_argument('--mode', choices=['preview', 'full'], default='full')
    ...
    from small_room30_v5_full import validate_run, FILES, TRAIN_FILES, original_case, verify_reviewed_path
    ...
    if (saved.get('schema') != TRAIN_SCHEMA or saved.get('purpose') != 'RESEARCH_TARGETED_V5_PILOT'
    ...
    manifest = dict(schema=SCHEMA, mode=args.mode, cases=cases,
        ...
        evaluation_code_sha256={n: sha(REPO / n) for n in EVAL_FILES + FILES + TRAIN_FILES},
        torch_version=str(torch.__version__), cuda_version=torch.version.cuda,
        device=torch.cuda.get_device_name(args.device),
        diffusion_steps=500, generation='free Gaussian noise; ancestral DDPM; no GT conditioning',
        frozen_base='reuse hash/GT/XYZ/seed-verified original full evaluation; no base rerun',
    ...
    try:
        ...
        rows = []; started=time.monotonic(); generated=0
        print('[EVAL] %d cases x saved1578 x 500 steps; verified Frozen Base reused; no training' % len(cases), flush=True)
        for num, case in enumerate(cases, 1):
            ...
            if receipt_path.exists():
                ...
                original=original_case(data,training['evaluation_summary'],case)
                if not np.array_equal(load_case(path)['base_raw'],original['base_raw']):raise ValueError('Cached Frozen Base differs')
                verify_reviewed_path(args.training_summary,case,load_case(path)['adapted_raw'])
                rows.append(row)
            ...
            original=original_case(data,training['evaluation_summary'],case)
            set_lora_enabled(runtime.model,True)
            raw=rollout(runtime,kw,initial,reverse,'%d/%d %s adapted1578'%(num,len(cases),case['sample_id']))
            verify_reviewed_path(args.training_summary,case,raw)
            maps=dict(base_raw=original['base_raw'],adapted_raw=raw)
            ...
            print('[PROGRESS] %d/%d cases; remaining approximately %.1f minutes'%(num,len(cases),(len(cases)-num)*elapsed/generated/60),flush=True)
        ...
        if sha(cp_path)!=cp_info['sha256']:raise ValueError('Checkpoint file changed during evaluation')
