#!/usr/bin/env python3
"""CPU/static contract tests for Teacher-v9.8.9 radius response."""
...
from relational_teacher_v989_rollout_aligned_radius_contract import (
    AUDIT_SCENE,
    POLICY,
    POLICY_ID,
    RADIUS_GRID,
...
def main() -> None:
    if canonical_sha256(POLICY) != POLICY_ID:
        raise AssertionError("rollout-aligned radius policy ID changed")
    if RADIUS_GRID != (0.00025, 0.0005, 0.001, 0.002, 0.003):
        raise AssertionError("radius grid changed")
    if SELECTED_DIRECTION != "audit_bed_guard_0p10":
        raise AssertionError("selected v9.8.8 direction changed")
    ...
    for literal in (
        ...
        if literal not in source:
            raise AssertionError("rollout-aligned radius runner lacks guard: " + literal)

    print("[PASS] Teacher-v9.8.9 rollout-aligned radius CPU contract")
    print("[PASS] audit regression and ineligible radius fail closed")
    print("[PASS] common-start, actual two-scene K=3 and no-checkpoint guards")
